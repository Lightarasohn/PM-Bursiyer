...
const BursiyerEkle = () => {
  ...
  // veya Next.js için: const router = useRouter();
  ...
  const getAllRequiredDocuments = async () => {
    try {
      ...
      console.log("Required Documents Constants:", requiredDocumentsConstants);
      ...
      requiredDocumentsConstants.forEach((item) => {
        ...
        const requiredDocumentsObjects = allDocuments
          .filter((x) => valueInt.includes(x.id))
          .map((item) => {
            return { label: item.name, value: item.id };
          });

        console.log(`${item.constantName} Documents:`, requiredDocumentsObjects);
  ...
  const handleFinish = async (values) => {
    ...
    try {
      ...
      values.termStartDate = values.termStartDate.format("YYYY-MM-DD"); // Bug düzeltildi
      ...
      const requestValues = {
        ...
        termStartDate: values.termStartDate, // Bug düzeltildi
      ...
      console.log("API Response:", response); // Debug için tüm response'u gör
      
      // Response farklı formatlarda olabilir
      let scholarId;
      
      if (typeof response === 'number') {
        // Eğer doğrudan ID dönüyorsa
        scholarId = response;
      } else if (response && response.id) {
        // Scholar object dönüyorsa (şu anki durum)
        scholarId = response.id;
      } else {
        // Hata durumu
      ...
      // Başarı mesajı gösterildikten sonra yönlendirme
      setTimeout(() => {
        // React Router ile yönlendirme
        navigate(`/scholar-info?targetID=${scholarId}`);
        
        // Eğer query parameter kullanıyorsanız:
        // navigate(`/scholar-info?id=${scholarId}`);
      }, 1500); // 1.5 saniye bekle ki kullanıcı başarı mesajını görsün
      
    } catch (error) {
      console.error("Error creating scholar:", error);
      messageApi.open({
        type: "error",
        content: "An error occurred while creating the scholar. Please try again.",
      });
    } finally {
      setIsSubmitting(false);
    }
    ...
    <>
      ...
      <div style={{ padding: '24px', maxWidth: '1200px', margin: '0 auto' }}>
        ...
        >
          ...
          >
            {/* Form içeriği aynı kalacak - sadece handleFinish fonksiyonu değişti */}
            <Row gutter={[24, 24]}>
                      ...
                      <Col span={12}>
                        <Form.Item
                          ...
                          rules={[
                            ...
                            ({ getFieldValue }) => ({
                              validator(_, value) {
                                const startDate = getFieldValue('termStartDate');
              ...
              <Col xs={24} md={8}>
                ...
                >
                  <Select
                    ...
                    size="large"
                    style={{ borderRadius: '6px' }}
                    options={requiredDocumentsOnEntry}
                    onChange={handleChange}
                    showSearch
                    filterOption={(input, option) =>
                      (option?.label ?? '').toLowerCase().includes(input.toLowerCase())
              ...
              <Col xs={24} md={8}>
                ...
                >
                  <Select
                    ...
                    size="large"
                    style={{ borderRadius: '6px' }}
                    options={requiredDocumentsOnExit}
                    onChange={handleChange}
                    showSearch
                    filterOption={(input, option) =>
                      (option?.label ?? '').toLowerCase().includes(input.toLowerCase())
              ...
              <Col xs={24} md={8}>
                ...
                >
                  <Select
                    ...
                    size="large"
                    style={{ borderRadius: '6px' }}
                    options={requiredDocumentsOnAttendance}
                    onChange={handleChange}
                    showSearch
                    filterOption={(input, option) =>
                      (option?.label ?? '').toLowerCase().includes(input.toLowerCase())
